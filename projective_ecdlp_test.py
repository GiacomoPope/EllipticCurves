...
def test(data, check_times, dlog=bsgs, profile=False):
    p = data["p"]
    a = data["a"]
    b = data["b"]
    # dlog secret
    ...
    if profile:
        import cProfile
        cProfile.runctx('pohlig_hellman(P, Q, n, n_factors)', {'P' : P, 'Q' : Q, 'n' : n, 'n_factors' : n_factors}, {'pohlig_hellman' : pohlig_hellman})
...
# test(easy, 1, dlog=bsgs, profile=False)
test(medium, 10, dlog=bsgs, profile=False)
